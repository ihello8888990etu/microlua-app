/*
 * lua_nn_class.c
 *
 *  Created on: 2019年3月30日
 *      Author: Administrator
 */
/*
 ** $Id: loslib.c,v 1.65.1.1 2017/04/19 17:29:57 roberto Exp $
 ** Standard Operating System library
 ** See Copyright Notice in lua.h
 */
#pragma pack(push)
#pragma pack(1)

#include <errno.h>
#include <locale.h>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <assert.h>
#include <modules.h>
#include <math.h>
DEV_HANDLE hSysNNFaceMaskHandle = NULL;

#define FACEMASKANCHOR_MAX 9


#if FACEMASKANCHOR_MAX == 5
static float facemaskanchor[FACEMASKANCHOR_MAX * 2] = {0.156250, 0.222548,0.361328, 0.489583,0.781250, 0.983133,1.621094, 1.964286,3.574219, 3.940000};
#endif
#if FACEMASKANCHOR_MAX == 9
static float facemaskanchor[FACEMASKANCHOR_MAX * 2] = {0.117188, 0.166667,0.224609, 0.312500,0.390625, 0.531250, 0.664062, 0.838196,0.878906, 1.485714,1.269531, 1.125714, 1.728516, 2.096633,2.787402, 3.200000,4.382959, 4.555469};
#endif


#define FACEMASK_KPU_WIDTH 320
#define FACEMASK_KPU_HEIGHT 256
#define FACEMASK_KPU_RESERVED_MEM 8*320
#define CLASS_NUMBER 2

LAYER_HANDLE gFaceMaskLayerHandle = NULL;

//----------------------------------------------------------------------------------
static int net_nn_facemask_tostring(FUN_HANDLE hFunHandle) {

	const char *szNetName = getNetworkName(hFunHandle);
	const char *szModName = getModuleName(hFunHandle);
	char szinfo[256] = { 0 };
	snprintf(szinfo, sizeof(szinfo), "NN ModName:%s, NetName:%s", szModName,
			szNetName);
	clear_toret(hFunHandle);
	setstring_toret(hFunHandle, szinfo, strlen(szinfo));
	return 1;
}

static int net_nn_facemask_init(FUN_HANDLE hFunHandle) {



	gettable_exit(hFunHandle);
	gFaceMaskLayerHandle = InitLayer(10, 8, (4 + 2 + 1) * FACEMASKANCHOR_MAX,
			FACEMASK_KPU_WIDTH, FACEMASK_KPU_HEIGHT, facemaskanchor, FACEMASKANCHOR_MAX,CLASS_NUMBER);
	assert(gFaceMaskLayerHandle != NULL);
	ParamLayer(gFaceMaskLayerHandle,0.7, 0.4);
	return 0;
}

//float *pred_box = NULL, *pred_landm = NULL, *pred_clses = NULL;
//size_t pred_box_size = 0, pred_landm_size = 0, pred_clses_size = 0;
//#define TEST 0

static int net_nn_facemask_forward(FUN_HANDLE hFunHandle) {

	IMG_HANDLE arg_other = getarg_toimage(hFunHandle, 2);

	float threshold = getarg_tonumber(hFunHandle, 3,0.7,true);
	float nms_value = getarg_tonumber(hFunHandle, 4, 0.4,true);
	obj_info_t draw_boxs;
	memset(&draw_boxs, 0, sizeof(obj_info_t));
	float *output = NULL;
	size_t output_size = 0;
	//printf("KPU_Run \r\n");
	const char *szNetName = getNetworkName(hFunHandle);
	assert(szNetName != NULL);

	assert(KPU_Run(szNetName, arg_other,FACEMASK_KPU_RESERVED_MEM,FACEMASK_KPU_WIDTH,FACEMASK_KPU_HEIGHT) == 0);


	int ret = KPU_GetOutput(szNetName, 0, &output, output_size);

	if (ret == 0) {

		ParamLayer(gFaceMaskLayerHandle,threshold,nms_value);
		RunLayer(gFaceMaskLayerHandle,output,output_size, &draw_boxs);

		if (draw_boxs.obj_number <= 0) {
			return 0;
		}

		clear_toret(hFunHandle);
		settable_toret(hFunHandle);
		//-----------------------------------------------------------------------------
		setinteger_totable(hFunHandle, "number", draw_boxs.obj_number);
		//-----------------------------------------------------------------------------
		settable_enter(hFunHandle, "boxs");
		for (int i = 0; i < draw_boxs.obj_number; i++) {
			settable_enter(hFunHandle, i + 1);
			setinteger_totable(hFunHandle, 1, draw_boxs.obj[i].x1);
			setinteger_totable(hFunHandle, 2, draw_boxs.obj[i].y1);
			setinteger_totable(hFunHandle, 3, draw_boxs.obj[i].x2-draw_boxs.obj[i].x1);
			setinteger_totable(hFunHandle, 4, draw_boxs.obj[i].y2-draw_boxs.obj[i].y1);
			settable_exit(hFunHandle);
		}
		settable_exit(hFunHandle);
		printf("number:%d x1:%d y1:%d x2:%d y2:%d\r\n", draw_boxs.obj_number,
				draw_boxs.obj[0].x1, draw_boxs.obj[0].y1, draw_boxs.obj[0].x2,
				draw_boxs.obj[0].y2);
		//----------------------------------------------------------------------------
		settable_enter(hFunHandle, "class");
		for (int i = 0; i < draw_boxs.obj_number; i++) {
			setinteger_totable(hFunHandle, i + 1, draw_boxs.obj[i].class_id+1);
		}
		settable_exit(hFunHandle);
		//----------------------------------------------------------------------------
		settable_enter(hFunHandle, "prob");
		for (int i = 0; i < draw_boxs.obj_number; i++) {
			setfloat_totable(hFunHandle, i + 1, draw_boxs.obj[i].prob);
		}
		settable_exit(hFunHandle);
		return 1;


	}
	return 0;
}

void module_nn_facemask_init() {

	//strncpy(lua_nn_facedetect.szModName,)
	hSysNNFaceMaskHandle = addNNModule("facemask",
			net_nn_facemask_tostring, net_nn_facemask_init,NULL);
	assert(hSysNNFaceMaskHandle != NULL);
	addFuntion(hSysNNFaceMaskHandle, "forward", net_nn_facemask_forward);
}

#pragma pack(pop)
